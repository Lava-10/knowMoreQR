...
class ProductNew extends React.Component<ProductNewProps, ProductNewState> {
  ...
  handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const { knowMoreQRtag } = this.state;
    knowMoreQRtag['sizeChart'] = sizeChart;
    knowMoreQRtag['colourways'] = colourways;
    knowMoreQRtag['media'] = media;
    knowMoreQRtag['stories'] = stories;
    knowMoreQRtag['itemFeatures'] = itemFeatures;
    this.setState({ knowMoreQRtag });

    const postTag = {
      ...this.state.knowMoreQRtag,
      companyId: "a516ef1b-4f86-4c92-b1d3-2377768b7faf",
      views: 0,
      saves: 0
    }

    axios.post('http://knowMoreQR-env.eba-2hmwrpmh.us-east-2.elasticbeanstalk.com/tags', postTag)
    .then(res => {
          ...
          <div className="mt-2 columns w-100 mx-0 is-flex-wrap-wrap">
            <div className="column is-full is-two-thirds-fullhd">
              ...
              <div className="w-100 is-flex is-justify-content-start">
                <button className="button custom-button py-2 mx-2 my-6" type="submit"><span className="mt-1 mx-3">CREATE knowMoreQRTAG</span></button>
              </div>
            </div>
