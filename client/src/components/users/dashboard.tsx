...
interface ProductsState {
  recentTags: knowMoreQRtag[];
  wishlistTags: knowMoreQRtag[];
}
...
class Dashboard extends React.Component<ProductsProps, ProductsState> {
  ...
  async populateTags(arrTagIds: string[]) {
    ...
    await Promise.all(arrTagIds.map(async function(tagId) {
      await axios.get(`http://knowMoreQR-env.eba-2hmwrpmh.us-east-2.elasticbeanstalk.com/tags/${tagId}`)
      .then(res => {
    ...
    return knowMoreQRtagsNew;
  }

  componentDidMount() {
    axios.get(`http://knowMoreQR-env.eba-2hmwrpmh.us-east-2.elasticbeanstalk.com/consumers/6363c590-2b22-4cbe-af9f-862b5b2cc2e0`)
    .then(async res => {
