...
class ProductView extends React.Component<ProductProps, ProductState> {
  ...
  componentDidMount() {
    ...
    axios.get(`http://api.knowMoreQR.info/tags/${id}`)
    .then(res => {
      const knowMoreQRtag = res.data.data;
      this.setState({ knowMoreQRtag });
      console.log(this.state.knowMoreQRtag);
    });
  }
        ...
        <div className="container mt-6 container-custom">
          <div className="columns mt-6 mb-4 w-100 mx-0 is-flex-wrap-wrap">
            <div className="column is-6-desktop is-full-mobile">
              <div className="product-image-container mx-5">
                <img className="product-image" src={this.state.knowMoreQRtag.colourways[this.state.activeColorIndex][2]} alt="Product preview"/>
              </div>
            </div>
            <div className="column is-6-desktop is-full-mobile has-text-left px-5">
              <div className="title is-size-4 has-text-theme-green-1">{this.state.knowMoreQRtag.name}</div>
              <div className="collection pt-3 pb-4">{this.state.knowMoreQRtag.series}</div>
              <hr/>
              <div className="is-size-6 is-text-centered-touch has-text-theme-green-1 has-text-weight-light opacity-60">{this.state.knowMoreQRtag.colourways[this.state.activeColorIndex][0]}</div>
              <div className="is-flex is-justify-content-center-touch w-100 mt-2 mb-5 pb-3">
              {this.state.knowMoreQRtag.colourways.map((colour, i) => {
                      ...
                      <table className="table is-hoverable mt-5 mb-6 border-radius-8 w-100">
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[0][0]}</td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[0][1]}</td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[0][2]}</td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[0][3]}</td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[0][4]}</td>
                          </tr>
                          <tr>
                            <td className="pt-2 px-4 has-background-grey-lighter has-text-centered"><div className="pt-2 is-size-6">WAIST</div></td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[1][0]}</td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[1][1]}</td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[1][2]}</td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[1][3]}</td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[1][4]}</td>
                          </tr>
                          <tr>
                            <td className="pt-2 px-4 has-background-grey-lighter has-text-centered"><div className="pt-2 is-size-6">SLEEVE</div></td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[2][0]}</td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[2][1]}</td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[2][2]}</td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[2][3]}</td>
                            <td className="pt-3 has-text-centered">{this.state.knowMoreQRtag.sizeChart[2][4]}</td>
                          </tr>
                        </tbody>
                      </table>
                    </section>
              ...
              <div className="description is-size-7 has-text-grey has-text-weight-light">
                {this.state.knowMoreQRtag.description}
                <br/><br/>
                {this.state.knowMoreQRtag.materials}
                <br/><br/>
                {this.state.knowMoreQRtag.instructions}
              </div>
            </div>
          </div>
          <div className="has-text-left mx-5">
            ...
            <div className="is-size-7 has-text-grey has-text-weight-light">
              {this.state.knowMoreQRtag.itemFeatures[0]}
              <br/>
            </div>
            <div className="columns my-5 mx-0 w-100 is-flex-wrap-wrap">
              <div className="column is-one-third-desktop is-full-touch">
                <div className="px-2 feature-box p-5">
                  <div className="is-size-5 mb-3">{this.state.knowMoreQRtag.itemFeatures[1]}</div>
                  <div className="is-size-7 has-text-grey has-text-weight-light">
                    {this.state.knowMoreQRtag.itemFeatures[2]}
                  </div>
                </div>
              </div>
              <div className="column is-one-third-desktop is-full-touch">
                <div className="px-2 feature-box p-5">
                  <div className="is-size-5 mb-3">{this.state.knowMoreQRtag.itemFeatures[3]}</div>
                  <div className="is-size-7 has-text-grey has-text-weight-light">
                    {this.state.knowMoreQRtag.itemFeatures[4]}
                  </div>
                </div>
              </div>
              <div className="column is-one-third-desktop is-full-touch">
                <div className="px-2 feature-box p-5">
                  <div className="is-size-5 mb-3">{this.state.knowMoreQRtag.itemFeatures[5]}</div>
                  <div className="is-size-7 has-text-grey has-text-weight-light">
                    {this.state.knowMoreQRtag.itemFeatures[6]}
                  </div>
                </div>
              </div>
            </div>
          </div>
          {this.state.knowMoreQRtag.media.map(function(el, i) {
            return(
          ...
          <div className="has-text-left mx-5 mb-6">
            ...
            <hr/>
            <div className="columns my-5 mx-0 w-100 is-flex-wrap-wrap">
            {this.state.knowMoreQRtag.stories.map(function(story, i) {
              return(
